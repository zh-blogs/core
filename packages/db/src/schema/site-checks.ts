...
export const SiteChecks = pgTable(
  'site_checks',
  {
    ...
    /** 检测时间 */
    check_time: timestamp({ withTimezone: true, precision: 6 }).notNull().defaultNow(),
  },
  (table) => [
    index('site_checks_site_id_check_time_index').on(table.site_id, table.check_time.desc()),
    index('site_checks_site_id_region_check_time_index').on(
      table.site_id,
      table.region,
      table.check_time.desc(),
    ),
    index('site_checks_result_check_time_index').on(table.result, table.check_time.desc()),
    ...
    check(
      'site_checks_status_code_valid_range_check',
      sql`${table.status_code} is null or (${table.status_code} >= 100 and ${table.status_code} <= 599)`,
    ),
    check(
      'site_checks_response_time_non_negative_check',
      sql`${table.response_time_ms} is null or ${table.response_time_ms} >= 0`,
    ),
    check(
      'site_checks_duration_non_negative_check',
