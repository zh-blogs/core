...
export const readPresenceCount = async (
  ...
  await customCommand(['ZREMRANGEBYSCORE', PRESENCE_KEY, '-inf', String(activeSince - 1)]);

  const result = await customCommand(['ZCOUNT', PRESENCE_KEY, String(activeSince), '+inf']);
...
export const createPresenceSnapshot = (count: number, now = new Date()): PresenceSnapshot => ({
  count,
  at: now.toISOString(),
});
...
export const formatPresenceComment = (comment = 'keepalive'): string => `: ${comment}\n\n`;
